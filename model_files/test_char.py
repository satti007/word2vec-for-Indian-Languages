...
def get_wordSyallables(sim_file,syl2idx,max_syl,pad_syl,lang):
	...
	for idx,l in enumerate(data):
		...
		w1_syl  = list(re_tel.sub(r'',w1))
		w2_syl  = list(re_tel.sub(r'',w2))
		if len(w1_syl) <= 7 and len(w2_syl) <= 7 and len(w1_syl) >= 4 and len(w2_syl) >= 4:
...
lang  = 'te'
max_syl  = 7
syl2idx  = loadFile(data_dir +'char2idx.pkl')
pad_syl  = len(syl2idx)
...
with train_graph.as_default():
	...
	max_1 = cnnLayer(4,embd_dim,input_p,filters,max_syl)
	max_2 = cnnLayer(5,embd_dim,input_p,filters,max_syl)
	max_3 = cnnLayer(6,embd_dim,input_p,filters,max_syl)
	max_4 = cnnLayer(7,embd_dim,input_p,filters,max_syl)
...
file = open('epoch_log.txt','wb',0)
for state in range(1,end+1):
	with tf.Session(graph=train_graph) as sess:
		sess.run(tf.global_variables_initializer())
		f = np.load('weights/weights_{}.npz'.format(state))
